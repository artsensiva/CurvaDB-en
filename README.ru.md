...
сжатию для потребительского GPS, а после того как step8 закрыл вопрос,
оставшийся от step3, — и для точных данных: даже сплайн со свободными
узлами, посаженный прямо на истинную кривую, не компактнее DP+SED на
20% ни при одном проверенном допуске (см. гипотезу H1, теперь закрытую,
в [docs/ru/findings.md](docs/ru/findings.md)). Это исследование
завершается — следующий шаг для него не код, а интервью с отраслью (см.
[docs/ru/next_steps.md](docs/ru/next_steps.md)).

**Статус проекта: исследование завершено и опубликовано** — код, полный технический отчёт,
23 записи об архитектурных решениях и результаты каждого этапа открыты на GitHub, с DOI для
цитирования (см. «Цитирование» выше). Адаптации для внешней публикации подготовлены
(`docs/devto_article.md`, `docs/ru/habr_article.md`), но пока нигде не опубликованы. Фазы 3-7
(выделение ядра `certigeo`, продукты P1-P4) не начаты и не начнутся, пока интервью с отраслью
выше не подтвердят спрос — точный статус по каждым воротам см. в `docs/ROADMAP.md`, раздел 8.
...
- Цена отказа от гарантии, измеренная явно: приближённый поиск без сертификатов по тем же сжатым
  данным всё равно даёт **0.03-3.03%** пропусков истинных ответов и до **2.04%** лишних.
...
- [docs/ru/findings.md](docs/ru/findings.md) — итоги исследования: вопрос,
  ключевые цифры, вывод, ограничения, закрытая гипотеза H1, открытая гипотеза H2.
...
- [docs/ru/blog_step7.md](docs/ru/blog_step7.md) — продолжение поста, про
  step7 (сертифицированное хранилище кривых).
