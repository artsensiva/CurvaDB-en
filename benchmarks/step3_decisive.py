    ...
    return "\n".join(
        [
            ...
            (
                ...
                "узлов m с консервативным потолком (>= k+2 отсчётов на "
                "узловой промежуток в среднем) -- без него рост m к почти-"
                "интерполяции даёт численный разрыв ошибки на зашумлённых "
                "данных (см. коммит модуля). Честная ошибка внутри фиттера -- "
                "ПРЯМОЙ Евклидов остаток в самих точках (t_i, xy_i), не "
                "point-to-segment до ломаной.\n"
            ),
            (
            ...
            (
                ...
                "точках) -- непрерывная бисекция (`_bisect_reachable` в "
                "step3_decisive.py), а не лог-сетка, как в step2. Размер -- "
                "единая схема для всех методов: время/координаты -> "
                "округление до см/сантисекунд -> int32 -> дельты -> "
                "zlib.compress.\n"
            ),
        ]
    ) + "\n"
