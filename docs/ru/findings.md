...
ломаной. На тот момент это НЕ было проверкой оптимальной/адаптивной
расстановки узлов (см. "Вывод" ниже) — этот вопрос стал гипотезой H1,
впоследствии закрытой step8 (см. "Закрытые гипотезы" ниже).
...
приблизить критерий K1 (30% сжатия). На тот момент это не проверялось
— так возникла гипотеза H1, теперь закрытая: свободный оракул step8
(раздел 2.6 спецификации) проверил именно это и снова не нашёл
выигрыша (см. "Закрытые гипотезы" ниже). При этом даже
...
расстановке узлов на тот момент не проверялась — так возникла гипотеза
H1, теперь закрытая (см. "Закрытые гипотезы" ниже).

## Закрытые гипотезы

### H1 (сжатие): свободные/оптимальные узлы на точных данных — ЗАКРЫТА (step8)

Адаптивная расстановка узлов по собственной геометрии истинной кривой
(не равномерная, как в оракуле step3, и не по остаткам шумного
пробного фита, как в `fit_adaptive`) могла бы в принципе дать более
компактное представление на ТОЧНЫХ (безшумных) данных, чем DP+SED,
размещая узлы там, где геометрия действительно сложнее — вопрос
остался открытым после step3, чей собственный оракул проверял только
равномерные узлы.

**Проверена и закрыта в step8** (`docs/specs/step8_A_hybrid.md`,
свободный оракул раздела 2.6: жадное сертифицированное удаление узлов
прямо на плотной выборке истинной кривой — `src/traj/knot_removal.py`,
раздел M1 в `benchmarks/results/step8.md`, критерий A3). Даже эта
свободная, геометрически обусловленная расстановка узлов не даёт
требуемых 20% выигрыша над DP+SED: ни при одном проверенном допуске
отношение байт оракул/DP+SED не укладывается в 0.80x (tol=0.5 м:
1.050x; tol=2 м: 0.834x, ближайший промах; tol=10 м: 0.879x). Правило
раздела 8 спецификации, применённое дословно: гипотеза H1 закрыта —
даже сплайн со свободными узлами на идеальной кривой не компактнее
DP+SED на 20% в проверенных условиях. Подробности и методика единого
основания сравнения: `docs/reviews/step8_M1.md`,
...
- **Сертификат по непрерывной метрике Фреше — это гарантия формы, а не
  гарантия синхронности по времени.** Он допускает, что реконструкция
  опережает или отстаёт от исходной кривой во времени, пока обе
  остаются близки в пространстве — это законное, Фреше-оптимальное
  соответствие. Если приложению нужна синхронность (совпадение
  положения в ОДИН И ТОТ ЖЕ момент времени, а не просто в каком-то
  месте в какой-то момент), нужно использовать SED или ошибку L2, а не
  сертификат по Фреше. Найдено при закрытии H1 (step8,
  `docs/specs/step8_A_hybrid.md`, свободный оракул раздела 2.6):
  сертифицированное удаление узлов проходило на 100% (`eps_A<=tol`) при
  каждом проверенном допуске, а достижимость по критерию с
  синхронизацией по времени падала с 86.7% до 66.7% с ростом допуска —
  это свойство самой метрики Фреше, не специфичное для H1 или для
  конкретного фиттера.
